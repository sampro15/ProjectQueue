...
router.post('/remove', async (req, res) => {
    try {
        ...
        await (0, projectService_1.removeProject)(projectId);
        res.end();
    }
    catch (error) {
...
router.post("/setGlobalVars", async (req, res) => {
    try {
        ...
        await (0, projectService_1.setGlobalVarsQueue)(projectId, globalVars);
...
router.get("/getGlobalVars", async (req, res) => {
    try {
        ...
        const globalVars = await (0, projectService_1.getGlobalVarsQueue)(Number(projectId));
        res.json(globalVars);
    }
    catch (error) {
...
router.post('/switch', async (req, res) => {
    try {
        ...
        await (0, projectService_1.switchProjects)(projectId1, projectId2);
        res.end();
    }
    catch (error) {
        res.status(500).json({ error: (0, errorHandling_1.getErrorMessage)(error) });
...
router.get('/getATSummary', async (req, res) => {
    try {
        const data = await api_1.projectApi.getATReport();
        res.json(data);
    }
    catch (error) {
        res.status(500).json({ error: (0, errorHandling_1.getErrorMessage)(error) });
...
router.post('/setNextTaskGuid', async (req, res) => {
    ...
    await (0, projectService_1.setNextTaskGuid)(taskId, nextTaskGuid);
    res.end();
});
...
router.post('/setNextTaskGuidForCurrent', async (req, res) => {
    try {
        ...
        await (0, projectService_1.setNextTaskGuidForCurrentTask)(nextTaskGuid);
        res.end();
    }
    catch (error) {
        res.status(400).json({ error: (0, errorHandling_1.getErrorMessage)(error) });
    }
});
// /**
//  * @swagger
...
router.post('/addTasks', async (req, res) => {
    try {
        ...
        const result = await (0, projectService_1.addTasksToProject)(projectId, projectKey, tasks);
        SseManager_1.sseManager.sendTasksAdded(projectId, projectKey, tasks);
        res.json(result);
    }
    catch (error) {
