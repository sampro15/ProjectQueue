...
import { closeDatabase, getConnection } from "./db";
import { v4 as uuidv4 } from "uuid";

export async function insertToDatabase(queue: Queue): Promise<void> {
  let conn;
  try {
    ...
    const lastOrderRow = await conn.get(
      "SELECT COALESCE(MAX(order_index), -1) as last_order FROM projects"
    );
    let nextOrderIndex = lastOrderRow.last_order + 1;

    // Insert all projects first
    const projectStmt = await conn.prepare(
      "INSERT INTO projects (project_key, manager, global_vars, order_index, status) VALUES (?, ?, ?, ?, ?)"
    );
    ...
    for (const project of queue.projects) {
      const result = await projectStmt.run(
        project.project_key,
        project.manager,
        JSON.stringify(project.global_vars),
        nextOrderIndex++,
    ...
    // Insert all tasks
    const taskStmt = await conn.prepare(
      "INSERT INTO tasks (project_id, guid, type, name, comment, action, task_params, next_task_guid, is_active, status) VALUES (?, ?, ?, ?, ?, ?, json(?), ?, ?, ?)"
    );
    ...
    for (const project of queue.projects) {
      for (const task of project.tasks) {
        ...
        const result = await taskStmt.run(
          ...
          task.comment,
          task.action,
          JSON.stringify(task.task_params),
          task.next_task_guid,
          task.is_active,
          task.status || "pending"
        );
...
export async function getQueueQuery(
  limit?: number,
  offset?: number,
  since?: Date,
  conn?: any
  ...
  try {
    ...
    const totalResult = await conn.get(
...
export async function getGlobalVars(
  ...
  try {
    ...
    if (projectId) {
      const rows = await conn.all(
        `SELECT global_vars FROM projects WHERE id = ?`,
        projectId
      );
...
export async function setGlobalVars(
  projectId: number,
  globalVars: any
): Promise<void> {
  let conn;
  try {
    ...
    const stmt = await conn.prepare(
      `UPDATE projects SET global_vars = json(?) WHERE id = ?`,
...
export async function getTaskByGuid(guid: string): Promise<Task | undefined> {
...
export async function checkQueueIsEmpty(): Promise<number> {
  let conn;
  try {
    conn = await getConnection();
    const stmt = await conn.prepare(
...
export async function getNextPendingProject(): Promise<Project | null> {
  let conn;
  try {
    conn = await getConnection();
    await conn.exec("BEGIN IMMEDIATE;");
...
export async function addProject(project: Project): Promise<void> {
  ...
  try {
    ...
    // Insert project
    const projectStmt = await conn.prepare(
      "INSERT INTO projects (project_key, global_vars, order_index, status) VALUES (?, ?, json(?), ?, ?)"
    );
    const result = await projectStmt.run(
      project.project_key,
      JSON.stringify(project.global_vars),
    ...
    if (project.tasks && project.tasks.length > 0) {
      ...
      for (const task of project.tasks) {
        task.project_id = project.id!;
        const taskResult = await taskStmt.run(
          project.id,
          task.guid,
          task.type,
          task.name,
...
export async function removeAllProjectsQuery(): Promise<void> {
  let conn;
  try {
    conn = await getConnection();
...
export async function switchProjects(
  projectId1: number,
  projectId2: number
): Promise<void> {
  let conn;
  try {
    ...
    const stmt = await conn.prepare(
      "SELECT order_index FROM projects WHERE id = ?"
    );
...
export async function insertTaskLog(
  ...
  try {
    conn = await getConnection();
    const stmt = await conn.prepare(
      "INSERT INTO Logs (task_id, status, log_message, log_level) VALUES (?, ?, ?, ?)"
    );
...
export async function getLatestLogs(
  ...
  logLevel?: LogLevel,
  since?: Date
): Promise<{ logs: TaskLogView[]; total: number }> {
  let conn;
  try {
    conn = await getConnection();
    ...
    const totalResult = await conn.get(
      `SELECT COUNT(*) as count FROM V_TaskLogs ${whereClause}`,
...
export async function updateTaskNextGuid(
  taskId: number,
  successNextTaskGuid: string | null
): Promise<void> {
  let conn;
  try {
    conn = await getConnection();
    const stmt = await conn.prepare(
      "UPDATE tasks SET next_task_guid = ? WHERE id = ?"
    );
...
export async function recoverInterruptedProject(): Promise<void> {
  let conn;
  try {
    conn = await getConnection();
    await conn.exec("BEGIN IMMEDIATE;");
...
export async function rerunProjectByTaskId(
  projectId: number,
  taskId: number | null
): Promise<void> {
  let conn;
...
export async function updateNextTaskByName(
  taskId: number,
  ...
  try {
    ...
    const sourceTask = await conn.get(
    ...
    if (nextTaskName) {
      const targetTask = await conn.get(
        "SELECT guid FROM tasks WHERE project_id = ? AND name = ?",
        sourceTask.project_id,
        nextTaskName
      );
      if (!targetTask) {
        throw new Error(
          `Task with name "${nextTaskName}" not found in the same project`
        );
      }
...
export async function addTasksToProject(
  ...
  try {
    ...
    if (!project) {
      throw new Error(
        `Project ${
          projectId ? `with ID ${projectId}` : `with key "${projectKey}"`
        } not found`
      );
    }
    ...
    if (project.status === "completed") {
      throw new Error(
        `Project ${
          projectId ? `with ID ${projectId}` : `with key "${projectKey}"`
        } is completed, cant add tasks to completed projects`
      );
    }
...
export async function getEventReason(taskId: number): Promise<string | null> {
  let conn;
  try {
    conn = await getConnection();

    const result = await conn.get(
      `SELECT log_message FROM V_TaskLogs WHERE task_id = ? ORDER BY timestamp DESC LIMIT 1`,
      [taskId]
    );
...
export async function abortAllProjects(): Promise<void> {
  let conn;
  try {
    conn = await getConnection();
    await conn.exec("BEGIN IMMEDIATE;");
