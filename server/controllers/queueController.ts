...
import {
  ...
  unpauseQueue,
  rerunProject,
  abortQueue as abortProject,
  removeAllProjects,
} from "../services/queueService";
import { getLatestLogs, insertTaskLog } from "../database/queries";
...
router.get("/getQueue", async (req, res) => {
  try {
    ...
    const queue = await getQueue(limit, offset, since);
    if (queue) {
      res.json(queue);
    } else {
...
router.get("/logs", async (req, res) => {
  try {
    const limit = req.query.limit ? parseInt(req.query.limit as string) : 100;
    const offset = req.query.offset ? parseInt(req.query.offset as string) : 0;
    const logLevel = req.query.level as LogLevel | undefined;
    ...
    const { logs, total } = await getLatestLogs(limit, offset, logLevel, since);
...
router.post("/rerun", async (req, res) => {
  try {
    ...
    const result = await rerunProject(projectId, taskId);
    res.json(result);
  } catch (error) {
    res.status(500).json({
...
router.post("/removeAll", async (req, res) => {
  try {
    const result = removeAllProjects();
    res.json({ success: result });
  } catch (error) {
...
router.post("/log", async (req, res) => {
    try {
        ...
        await insertTaskLog(taskId, status, message, logLevel);
